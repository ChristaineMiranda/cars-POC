...
async function updateCar(req: Request, res: Response) {
  ...
  const {model, licensePlate, year, color} = req.body as {model: string, licensePlate: string, year: number, color: string}; 
  try {
    await carService.updateCar(Number(carId), model, licensePlate, year, color);
